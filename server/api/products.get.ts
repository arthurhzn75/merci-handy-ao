...
export default defineEventHandler(async (event) => {
  ...
  if (query.excludeGifts === 'true') {
    catalog = catalog.filter(p => !p.isGift)
  }
  if (query.type) {
    catalog = catalog.filter(p => p.type === query.type)
  }
  if (query.abc) {
    catalog = catalog.filter(p => p.abcClass === query.abc)
  }
  if (query.search) {
  ...
  // Recompute ABC after filtering (since subset changes classification)
  ...
  // All available types (from unfiltered data)
